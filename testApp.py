...
# Define the primary and secondary colors (Ensure this is at the top)
PRIMARY_COLOR = "#00BCD4"  # Teal color matching your logo
SECONDARY_COLOR = "#E0E0E0"  # Light gray for secondary buttons
TEXT_COLOR = "#FFFFFF"  # White text for primary buttons
FONT = ("Helvetica", 8)

# Function to style buttons
def style_button(button, primary=True):
    button.configure(
        bg=PRIMARY_COLOR if primary else SECONDARY_COLOR,
        fg=TEXT_COLOR if primary else "#000000",
        activebackground="#008C9E" if primary else "#B0BEC5",
        font=FONT,
        bd=0,
        relief="flat",
        cursor="hand2",
...
class SCDWhatnotGUI:
    def __init__(self, root):
        ...
        try:
            ...
            # Only create config.ini if it does not exist
            ...
            # Read existing config without overwriting
            self.config.read(self.config_path)

            # Subscription tier will now be loaded securely from the database
            self.user_email = "user_email@example.com"  # Replace with actual user email
            self.load_subscription()  # Securely load subscription from SQLite

            # Ensure sections exist in config.ini without overwriting user settings
            for section in ['Telegram', 'GUI']:
                ...
                if section == 'Telegram':
                    ...
                    self.config[section].setdefault(
                        'top_buyer_text', 
                        "Great job, {username}! You've snagged {count} items!"
                    )
                    self.config[section].setdefault(
                        'giveaway_announcement_text',
                        "Giveaway #{number} Alert! Must be following us & share the stream to enter! "
            ...
            # Save updated config only if new settings were added
            with open(self.config_path, "w", encoding="utf-8") as configfile:
                self.config.write(configfile)

            # Initialize subscription attributes securely
            self.user_email = "user_email@example.com"  # Replace with actual user email
            self.load_subscription()  # Load the subscription securely from database

            # Load other GUI and Telegram settings from config.ini
            ...
            # Set subscription variable for GUI
                    ...
                    if event['type'] == 'checkout.session.completed':
                        ...
                        if not sub_id:
                            ...
                            if new_tier:
                                ...
                                self.update_subscription(new_tier, self.user_email)
            ...
            # Initialize Flask and SocketIO (secure local server)
            self.flask_app = Flask(__name__)
            self.socketio = SocketIO(self.flask_app, cors_allowed_origins="*", async_mode="threading")
            self.latest_bin_assignment = "Waiting for bidder..."

            def run_flask():
                """Run the Flask server in a separate thread securely."""
                try:
                    port = int(os.getenv('PORT', 5000))
                    self.socketio.run(
                        self.flask_app,
                        host='127.0.0.1',  # Secure - Local access only
                        port=port,
                        debug=False
                    )
            ...
            try:
                ...
                if not self.telegram_error_shown:  # Prevents repeated popups
                    messagebox.showerror("Style Error", f"Failed to configure styles. Check logs for details.")

            # Header Frame (Top of GUI)
            ...
            try:
                logo_img = Image.open(logo_path).resize((160, 50), Image.LANCZOS)  # Updated to Image.LANCZOS for compatibility
            ...
            # Allow notebook to resize with the window
            root.grid_rowconfigure(1, weight=1)  # Notebook row resizes
            root.grid_columnconfigure(0, weight=1)  # Left column resizes
            root.grid_columnconfigure(1, weight=1)  # Right column resizes

            # Initialize Tabs (Example: Bidders, Settings)
            self.bidders_tab = tk.Frame(self.notebook)
            ...
            # Initializing empty tabs, ready for content
            # Transactions Tab (Optimized)
            self.transactions_tab = tk.Frame(self.notebook, bg="#FFFFFF")
            self.notebook.add(self.transactions_tab, text="Transactions")

            # Input Frame (For Search and Bidder Management)
            input_frame = tk.Frame(self.transactions_tab, bg="#FFFFFF")
            input_frame.pack(fill="x", padx=5, pady=5)
            input_frame.grid_columnconfigure(1, weight=1)

            # Announcement Text (Dynamic Display)
            ...
            self.search_entry.grid(row=1, column=1, padx=5, sticky="nsew")
            ...
            tk.Label(input_frame, text="Username:", bg="#FFFFFF", fg=PRIMARY_COLOR, font=("Helvetica", 10, "bold")).grid(row=2, column=0, padx=5, sticky="e")
            ...
            add_bidder_button.grid(row=2, column=3, padx=5, pady=10)

            # Quantity Label and Entry (Styled)
            tk.Label(input_frame, text="Quantity:", bg="#FFFFFF", fg=PRIMARY_COLOR, font=("Helvetica", 8)).grid(row=3, column=0, padx=5, sticky="e")
            self.qty_entry = tk.Entry(input_frame, width=15, font=("Helvetica", 8), bg="#F5F5F5")
            self.qty_entry.insert(0, "1")
            self.qty_entry.grid(row=3, column=1, padx=5)

            # Weight Label and Combobox (Styled)
            tk.Label(input_frame, text="Weight:", bg="#FFFFFF", fg=PRIMARY_COLOR, font=("Helvetica", 8)).grid(row=3, column=3, padx=5, sticky="e")
            ...
            avg_sell_rate_button.grid(row=5, column=0, padx=5, pady=5)
            ...
            start_giveaway_button.grid(row=5, column=2, padx=5, pady=5)
            ...
            start_flash_sale_button.grid(row=5, column=4, padx=5, pady=5)
            ...
            sort_asc_button.grid(row=6, column=0, padx=5, pady=5)
            ...
            sort_desc_button.grid(row=6, column=1, padx=5, pady=5)

            # Export and Import Buttons (Styled)
            ...
            # Configuring Treeview Columns (Error-Free)
            ...
            self.update_bin_display()  # Dynamically update bins
            ...
            # Subscription Management (Dynamic)
            ...
            self.tier_combobox.set(self.tier)  # Set to current tier
            ...
            # Dynamic Top Buyers Labels (Error-Free)
            ...
            # Dynamic Stats Text (Auto-Updated)
            ...
            # Dynamic Sell Rate Text (Auto-Updated)
            ...
            # Dynamic Footer Label (Auto-Updating)
            ...
            # Method to Dynamically Update Footer
            self.update_footer()

            # Initialize UI updates (Optimized)
            try:
                self.update_footer()
                ...
                logging.error(f"Failed to initialize UI updates: {e}", exc_info=True)
            ...
            messagebox.showerror(" 초기화 오류", f"UI 초기화 실패: {e}")
            self.root.destroy()

    def load_subscription(self):
        """Load the user's subscription from the config file."""
        try:
            # Ensure Subscription section exists in config
            if 'Subscription' not in self.config:
                self.config['Subscription'] = {}

            # Load subscription tier and license key, defaulting to Trial
            self.tier = self.config['Subscription'].get('tier', 'Trial')
            self.max_bins = self.tier_limits.get(self.tier, {'bins': 150})['bins']
            self.license_key = self.config['Subscription'].get('license_key', '')

            # If no subscription, default to Trial
            if not self.tier:
                self.tier = "Trial"
                self.max_bins = self.tier_limits['Trial']['bins']
                self.config['Subscription']['tier'] = self.tier
                self.config['Subscription']['license_key'] = ''
                self.write_config()
    ...
    def write_config(self):
        """Centralized method to write config.ini, reducing redundant writes."""
        try:
    ...
    def update_subscription(self, tier, license_key):
        ...
        self.update_ui()  # Auto-update UI
        logging.info(f"Subscription updated: Tier={tier}, License Key={license_key}")

    def update_footer(self):
        """Dynamically update the footer with accurate bin count and subscription details."""
        assigned_bins = len(self.bidders)
        self.footer_label.config(
            text=f"License ID: SS-2025-001 | SwiftSale(TM) (C) 2025 | Bins: {assigned_bins}/{self.max_bins}"
        )
        logging.info("Footer updated.")
    ...
    def update_bin_display(self):
        ...
        if not self.bidders:
            self.bin_display.insert(tk.END, "No bins assigned yet.")
        else:
            bin_text = "\n".join(
